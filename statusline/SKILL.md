...
argument-hint: "[what to show, for example 'dev server + open queue count']"
...
separator), and runs it under a 5s timeout (errors ignored, so a broken extension
cannot break the bar).

So: **declare links, do not print them**, and print only status to stdout.
...
1. **`$CL_ROOT/.claude/statusline-ext.sh`**: committed. Inert without the core,
   so safe to commit, and the natural home for *project* knowledge.
...
2. **Pick the place**: registry by default, and in-repo only if asked (§ above).
3. **Write the extension.** Start from the skeleton below. Source `$CL_LIB`, read
   ...
   inline, and everything else through `cl_cache`). Declare clickable things with
   `cl_addlink`, and `printf` only the non-link status (with `CL_*` colours) to
   stdout. `chmod +x` it.
